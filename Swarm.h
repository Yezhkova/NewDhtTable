...
public:
    ...
    void addNewParticipants( int newNodeNumber )
    {
        ...
        for( int i=0; i<newNodeNumber; )
        {
            ...
            m_nodes.back().findNode( m_nodes.front(), m_nodes.front() );
            m_nodes.front().findNode( m_nodes.back(), m_nodes.back() );
    ...
    void colonize()
    {
        ...
        {
            it->findNode( m_nodes.front(), m_nodes.front() );
            m_nodes.front().findNode( *it, *it );
        }
...
//        }
        for( auto it = m_nodes.begin()+1; it != m_nodes.end(); it++ )
        {
            for( auto it2 = m_nodes.begin()+1; it2 != m_nodes.end(); it2++ )
            {
                if ( it != it2 )
                {
                    it2->findNode( *it, *it );
